...
export function InteroTimelineChart({
  ...
  const { paths, compositePath, timeLabels, tsMin, tsRange } = useMemo(() => {
    if (snapshots.length < 2) {
      return { paths: {}, compositePath: '', timeLabels: [], tsMin: 0, tsRange: 1 };
    }
      ...
      {hoverSnap && (
        <div
          ...
          style={{
            ...
            top: 4,
            transform: hoverX > width * 0.7 ? 'translateX(-100%)' : 'translateX(0)',
          }}
        >
          ...
          {AXIS_KEYS.map((key) => (
            <div key={key} className={styles.tooltipAxisRow}>
              ...
              />
              <span className={styles.tooltipAxisLabel}>{AXIS_LABELS[key]}:</span>
              <span className={styles.tooltipAxisValue}>
                {Math.round(hoverSnap.stressAxes?.[key as keyof typeof hoverSnap.stressAxes] ?? 0)}
              </span>
            </div>
          ))}
        </div>
