 ...
 */
export function parsePlannerAction(action: string, parameters: Record<string, unknown> = {}): string {
  ...
  // Fallback: format as action with parameters
  if (Object.keys(parameters).length > 0) {
    const paramDescriptions = Object.entries(parameters)
      .map(([key, value]) => `${key}: ${value}`)
      .join(', ');
    return `${action.charAt(0).toUpperCase() + action.slice(1)} (${paramDescriptions})`;
  }
 ...
 */
export function parseCurrentAction(action: string | Record<string, unknown>): string {
