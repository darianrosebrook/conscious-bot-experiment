...
export async function POST(request: NextRequest) {
  try {
    ...
    connections.forEach((controller) => {
      try {
        controller.enqueue(
          new TextEncoder().encode(`data: ${JSON.stringify({ event, data, timestamp: Date.now() })}\n\n`)
        );
      } catch (error) {
...
export async function GET() {
  const stream = new ReadableStream({
    start(controller) {
      ...
      controller.enqueue(
        new TextEncoder().encode(`data: ${JSON.stringify({ event: 'connected', message: 'Memory updates connected', timestamp: Date.now() })}\n\n`)
      );
