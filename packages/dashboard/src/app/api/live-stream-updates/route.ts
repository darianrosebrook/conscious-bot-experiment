...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'development' && process.env.DEBUG_API === 'true' && process.env.DEBUG_LIVE_STREAM === 'true') {
      const requestStart = request.headers.get('x-request-start');
      const duration = requestStart ? Date.now() - parseInt(requestStart) : 0;
      console.log(`[Dashboard] POST /api/live-stream-updates 200 in ${duration}ms`);
    }
