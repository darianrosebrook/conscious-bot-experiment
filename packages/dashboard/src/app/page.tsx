...
const PLACEHOLDER_STREAM_SRC = "https://images.unsplash.com/photo-1606313564200-e75d5e30476f?q=80&w=2060&auto=format&fit=crop";
...
export default function ConsciousMinecraftDashboard() {
  ...
  }, [thoughts.length, tasks.length, hud, environment, setThoughts, setTasks, setHud, setEnvironment]);
  ...
  const handleSubmitIntrusion = async () => {
    ...
    try {
      ...
      if (response.accepted) {
        ...
        console.error('Intrusive thought rejected:', response.rationale);
      }
    } catch (error) {
      console.error('Failed to submit intrusive thought:', error);
    }
  };
          ...
          <nav className="ml-6 hidden md:flex items-center gap-4 text-sm text-zinc-300">
            <Tabs defaultValue="live" className="w-auto">
              <TabsList className="bg-zinc-900/50">
                <TabsTrigger value="live" className="hover:text-zinc-100">Live</TabsTrigger>
                <TabsTrigger value="memories" className="hover:text-zinc-100">Memories</TabsTrigger>
                <TabsTrigger value="events" className="hover:text-zinc-100">Events</TabsTrigger>
                <TabsTrigger value="settings" className="hover:text-zinc-100">Settings</TabsTrigger>
              </TabsList>
            </Tabs>
          </nav>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="bg-zinc-900 border-zinc-800 hover:bg-zinc-800/80">
          ...
          >
            {isLive ? <PauseCircle className="size-4 mr-2" /> : <PlayCircle className="size-4 mr-2" />}
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
          {hud && (
            <>
              ...
              <HudMeter label="Stress" value={hud.intero.stress} hint="lower is better" />
            ...
            <ScrollArea className="max-h-[38vh]">
              <div className="flex flex-col gap-2 pr-1">
                {thoughts.map((thought) => (
                  <div key={thought.id} className="rounded-xl border border-zinc-800 bg-zinc-950 p-2.5">
                    <div className="flex items-center justify-between text-[11px] text-zinc-400">
                      <span className="uppercase tracking-wide">
                        {thought.type === "intrusion" ? "thought" : thought.type}
                      </span>
                      <time className="tabular-nums">{formatTime(thought.ts)}</time>
                    </div>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-2">
            <div className="flex items-center gap-2">
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") handleSubmitIntrusion(); }}
            ...
            <div className="mt-1 text-[11px] text-zinc-500">
              Debug tip: keep attribution hidden in prod so the bot treats the injection as self-generated.
            </div>
          </div>
        </main>
