...
function createEntityFilter() {
    ...
    return function filterEntity(entity) {
...
async function main() {
    ...
    console.log(` Connecting to ${config.serverAddress}:${config.serverPort} (${config.version})`);
    console.log(` Username: ${config.username}`);
    console.log(` Viewer will be available at http://localhost:${config.viewerPort}`);
    try {
        // Create the bot
        ...
        bot.on('spawn', () => {
            ...
            if (errorMessages.length > 0 || warningMessages.length > 0) {
                console.log(`⚠️ Suppressed ${errorMessages.length} unknown entity errors and ${warningMessages.length} warnings during viewer startup`);
            }
        });
        bot.on('login', () => {
...
//# sourceMappingURL=mc-viewer.js.map