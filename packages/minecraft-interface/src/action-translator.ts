...
export interface CollectDiagnostics {
  ...
  scan: {
    ...
    item_types_seen_count: number;
    /** Total item entities observed across all scanned positions. */
    entities_seen_total: number;
    closest_item_distance: number | null;  // quantized to 0.1 blocks
    item_types_seen: string[];             // sorted, max 20
    /** Whether the requested target item type was observed among item_types_seen. */
    target_type_seen: boolean;
  };
  explore: {
    enabled: boolean;
    /** Ground-level positions generated by the spiral before surface checks. */
    waypoint_candidates: number;
...
export class ActionTranslator {
    ...
    return {
      ...
      diagnostics: {
        ...
        scan: {
          ...
          closest_item_distance: null,
          item_types_seen: [],
          target_type_seen: false,
        },
        explore: {
          enabled: false,
          waypoint_candidates: 0,
          waypoints_planned: 0,
          waypoints_reached: 0,
          path_requests: 0,
          path_successes: 0,
  ...
  private async fallbackSpiralExploration(
    ...
    let pathSuccesses = 0;
    let pathFailures = 0;
    let waypointCandidates = 0;
    let entitiesSeenTotal = 0;
    ...
    for (let r = 2; r <= spiralRadius; r += 3) {
      ...
      for (const direction of directions) {
        ...
        try {
          ...
          for (const entity of allItemEntities) {
            entitiesSeenTotal++;
            const dist = entity.position.distanceTo(this.bot.entity.position);
            if (closestItemDist === null || dist < closestItemDist) {
              closestItemDist = dist;
            }
            const entityItemId = (entity.metadata?.[8] as any)?.itemId;
            if (entityItemId != null) {
              // Resolve item name from ID for diagnostics
              const mcData = (this.bot as any).mcData;
              const itemInfo = mcData?.items?.[entityItemId];
              itemTypesSeen.add(itemInfo?.name ?? `id:${entityItemId}`);
            }
          }
          ...
          if (itemsFound.length > 0) {
            ...
            return {
              ...
              diagnostics: {
                ...
                explore: {
                  enabled: true,
                  waypoint_candidates: waypointCandidates,
                  waypoints_planned: waypointsAttempted,
                  waypoints_reached: waypointsReached,
                  path_requests: pathRequests,
                  path_successes: pathSuccesses,
                  path_failures: pathFailures,
                  net_position_delta: quantize1(this.bot.entity.position.distanceTo(startPos)),
                  duration_ms: Date.now() - startTime,
                },
                reason_code: pickupResult.success ? 'collected_ok' : 'no_item_entities',
              },
            };
          }
        } catch (error) {
    ...
    const diagnostics: CollectDiagnostics = {
      ...
      reason_code: resolveCollectReasonCode(pathRequests, pathSuccesses, timedOut),
    };
