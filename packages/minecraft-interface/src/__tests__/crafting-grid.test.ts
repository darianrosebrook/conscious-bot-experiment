...
const mockBot = {
  ...
  recipesAll: vi.fn().mockReturnValue([
    {
      type: 'shaped',
...
describe('SimpleCraftingGridTester', () => {
  ...
  describe('Recipe Detection', () => {
    ...
    it('should categorize recipes by grid size', () => {
      ...
      const advancedRecipes = recipes.filter(
        (recipe: any) =>
...
describe('Crafting Scenario Validation', () => {
  ...
  it('should validate precondition parsing', () => {
    ...
    const logPrecondition =
      mockCraftingScenario.preconditions.inventory_oak_log;
  ...
  it('should validate success condition parsing', () => {
    const experimentsCondition =
      mockCraftingScenario.success_conditions.crafting_experiments_completed;
    const requiredExperiments = parseInt(
      experimentsCondition.replace('>=', '')
    );
