/**
 * worldView.js - WorldView with entity name in all entity events
 *
 * This is a patched version that includes entity name, width, height in
 * entityMoved events to prevent rendering fallbacks (cows as Steve).
 *
 * @module viewer/renderer/worldView
 * @author @darianrosebrook
 */

import { spiral, ViewRect, chunkPos } from '../utils/simpleUtils.js'
  ...
  listenToBot(bot) {
    ...
    this.listeners[bot.username] = {
      ...
      entityMoved: function (e) {
        // Include name, width, height so client can create mesh if entityMoved
        // arrives before entitySpawn (race condition). Without name, getEntityMesh
        // falls back to 'player', rendering cows/mobs as Steve.
        worldView.emitter.emit('entity', {
          id: e.id,
          name: e.name,
          pos: e.position,
