...
export interface ChatMessage {
  ...
  messageType: 'chat' | 'command' | 'question' | 'greeting' | 'request' | 'statement';
  intent?: string;
  emotion?: string;
...
const DEFAULT_CONFIG: ChatProcessorConfig = {
  ...
  allowedCommands: ['help', 'follow', 'stop', 'come', 'go', 'build', 'craft', 'mine'],
...
export class ChatProcessor extends EventEmitter {
  ...
  async processMessage(sender: string, content: string): Promise<void> {
    ...
    if (message.requiresResponse && this.config.enableAutoResponse) {
  ...
  private classifyMessageType(content: string): ChatMessage['messageType'] {
    ...
    if (lowerContent.startsWith('/') || this.isCommand(lowerContent)) return 'command';
  ...
  private extractIntent(content: string): string {
    ...
    if (lowerContent.includes('help') || lowerContent.includes('assist')) return 'request_help';
    if (lowerContent.includes('follow') || lowerContent.includes('come')) return 'request_follow';
    if (lowerContent.includes('stop') || lowerContent.includes('wait')) return 'request_stop';
    if (lowerContent.includes('build') || lowerContent.includes('construct')) return 'request_build';
    if (lowerContent.includes('craft') || lowerContent.includes('make')) return 'request_craft';
    if (lowerContent.includes('mine') || lowerContent.includes('dig')) return 'request_mine';
    if (lowerContent.includes('hello') || lowerContent.includes('hi')) return 'greeting';
    if (lowerContent.includes('goodbye') || lowerContent.includes('bye')) return 'farewell';
  ...
  private detectEmotion(content: string): string {
    ...
    if (lowerContent.includes('!') || lowerContent.includes('amazing') || lowerContent.includes('awesome')) return 'excited';
    if (lowerContent.includes('please') || lowerContent.includes('could you')) return 'polite';
    if (lowerContent.includes('urgent') || lowerContent.includes('quick')) return 'urgent';
    if (lowerContent.includes('sad') || lowerContent.includes('sorry')) return 'sad';
  ...
  private requiresResponse(content: string): boolean {
    ...
    return messageType === 'question' || messageType === 'command' || messageType === 'request' || messageType === 'greeting';
  }

  /**
   * Calculate response priority
   */
  private calculateResponsePriority(content: string): ChatMessage['responsePriority'] {
    const lowerContent = content.toLowerCase();
    
    if (lowerContent.includes('urgent') || lowerContent.includes('emergency')) return 'urgent';
    if (lowerContent.includes('help') || lowerContent.includes('please')) return 'high';
  ...
  private isRequest(content: string): boolean {
    const requestWords = ['can you', 'could you', 'would you', 'please', 'help', 'need', 'want'];
  ...
  private updatePlayerInteraction(sender: string, message: ChatMessage): void {
    ...
    if (interaction.interactionCount > 20) interaction.relationshipLevel = 'friend';
    else if (interaction.interactionCount > 5) interaction.relationshipLevel = 'acquaintance';
    
    // Update communication style based on message content
    if (message.content.includes('please') || message.content.includes('thank')) {
   ...
   */
  private async generateResponse(message: ChatMessage): Promise<ChatResponse | null> {
    const interaction = this.playerInteractions.get(message.sender);
    const responseDelay = Math.random() * (this.config.responseDelay.max - this.config.responseDelay.min) + this.config.responseDelay.min;
   ...
   */
  private generateQuestionResponse(message: ChatMessage, interaction?: PlayerInteraction): string {
    const lowerContent = message.content.toLowerCase();
   ...
   */
  private generateRequestResponse(message: ChatMessage, interaction?: PlayerInteraction): string {
    const lowerContent = message.content.toLowerCase();
   ...
   */
  private generateCommandResponse(message: ChatMessage, interaction?: PlayerInteraction): string {
    const command = this.extractCommand(message);
   ...
   */
  private generateGeneralResponse(message: ChatMessage, interaction?: PlayerInteraction): string {
    const responses = [
      'I see!',
      'That\'s interesting!',
      'Thanks for sharing!',
