...
function isSequenceNode(node) {
    return (node === null || node === void 0 ? void 0 : node.type) === 'Sequence' && Array.isArray(node.children);
}
/**
 * Type guard for SelectorNode
 */
function isSelectorNode(node) {
    return (node === null || node === void 0 ? void 0 : node.type) === 'Selector' && Array.isArray(node.children);
}
/**
 * Type guard for RepeatUntilNode
 */
function isRepeatUntilNode(node) {
    return (node === null || node === void 0 ? void 0 : node.type) === 'Repeat.Until' && node.child && node.condition;
}
/**
 * Type guard for TimeoutDecoratorNode
 */
function isTimeoutDecoratorNode(node) {
    return ((node === null || node === void 0 ? void 0 : node.type) === 'Decorator.Timeout' &&
        node.child &&
...
function isFailOnTrueDecoratorNode(node) {
    return (node === null || node === void 0 ? void 0 : node.type) === 'Decorator.FailOnTrue' && node.child && node.condition;
}
/**
 * Type guard for LeafNode
 */
function isLeafNode(node) {
    return (node === null || node === void 0 ? void 0 : node.type) === 'Leaf' && typeof node.leafName === 'string';
}
/**
...
function getLeafNames(node) {
    ...
    if (isLeafNode(node)) {
        ...
        node.children.forEach(function (child) { return names.push.apply(names, getLeafNames(child)); });
    }
    else if (isRepeatUntilNode(node)) {
        names.push.apply(names, getLeafNames(node.child));
    }
...
function getSensorPredicates(node) {
    ...
    if (isRepeatUntilNode(node)) {
        ...
        predicates.push.apply(predicates, getSensorPredicates(node.child));
    }
        ...
        node.children.forEach(function (child) {
            return predicates.push.apply(predicates, getSensorPredicates(child));
        });
    }
...
function validateBTDSL(node) {
    ...
    function validateNode(n, path) {
        ...
        if (!isBTNode(n)) {
            errors.push("".concat(path, ": Invalid node structure"));
            return;
        }
        ...
        if (!allowedTypes.includes(n.type)) {
            errors.push("".concat(path, ": Disallowed node type '").concat(n.type, "'"));
            return;
        }
        // Validate specific node types
        if (isSequenceNode(n) || isSelectorNode(n)) {
            if (!n.children || n.children.length === 0) {
                errors.push("".concat(path, ": ").concat(n.type, " must have at least one child"));
            }
            else {
                n.children.forEach(function (child, index) {
                    return validateNode(child, "".concat(path, ".children[").concat(index, "]"));
                });
            }
        }
        else if (isRepeatUntilNode(n)) {
            validateNode(n.child, "".concat(path, ".child"));
            if (!isSensorPredicate(n.condition)) {
                errors.push("".concat(path, ".condition: Invalid sensor predicate"));
            }
        }
        else if (isTimeoutDecoratorNode(n)) {
            validateNode(n.child, "".concat(path, ".child"));
            if (n.timeoutMs <= 0) {
                errors.push("".concat(path, ".timeoutMs: Must be positive"));
            }
        }
            ...
            if (!n.leafName || n.leafName.trim() === '') {
                errors.push("".concat(path, ".leafName: Must be non-empty"));
            }
        }
    }
    validateNode(node);
