...
exports.SimulatedMinecraftInterface = exports.DEFAULT_SIMULATION_CONFIG = void 0;
...
class SimulatedMinecraftInterface extends events_1.EventEmitter {
    ...
    generateRandomBlocks() {
        ...
        for (let x = 0; x < this.config.worldSize.width; x += 2) {
            for (let z = 0; z < this.config.worldSize.depth; z += 2) {
                ...
                const type = this.config.blockTypes[Math.floor(Math.random() * this.config.blockTypes.length)];
    ...
    generateRandomEntities() {
        const entities = [];
        for (let i = 0; i < 5; i++) {
            const type = this.config.entityTypes[Math.floor(Math.random() * this.config.entityTypes.length)];
    ...
    async executeAction(action) {
        ...
        try {
            switch (action.type) {
                ...
                case 'place_block':
                    return this.executePlaceBlock(action.parameters?.position, action.parameters?.blockType);
                default:
                    return {
                        success: false,
    ...
    executeMineBlock(position) {
        ...
        const blockIndex = this.gameState.blocks.findIndex((block) => block.x === targetPos.x &&
            block.y === targetPos.y &&
            block.z === targetPos.z);
        if (blockIndex >= 0) {
    ...
    executePlaceBlock(position, blockType) {
        ...
        const inventoryIndex = this.gameState.inventory.findIndex((item) => item.id === type);
        if (inventoryIndex >= 0) {
    ...
    startSimulation() {
        this.tickInterval = setInterval(() => {
            ...
            if (this.gameState.time % 100 === 0) {
                this.gameState.weather = this.config.blockTypes[Math.floor(Math.random() * 3)];
            }
