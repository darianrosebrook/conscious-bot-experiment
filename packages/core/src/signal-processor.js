...
exports.SignalProcessor = exports.DEFAULT_SIGNAL_CONFIG = void 0;
 ...
 */
const DEFAULT_AGGREGATION_RULES = [
    {
        needType: 'safety',
        signalWeights: {
            threat: 0.4,
 ...
 */
class SignalProcessor extends events_1.EventEmitter {
    constructor(config = exports.DEFAULT_SIGNAL_CONFIG, customRules) {
        super();
        this.config = config;
        this.currentNeeds = new Map();
        this.lastNeedUpdate = 0;
        this.signalHistory = new types_1.BoundedHistory(this.config.historySize);
    ...
    processSignal(signal) {
        ...
        if (this.config.constitutionalFiltering &&
            !this.passesConstitutionalFilter(validatedSignal)) {
            console.warn(`Signal filtered out by constitutional rules: ${signal.type}`);
            return;
        }
        // Add to history
     ...
     */
    passesConstitutionalFilter(signal) {
     ...
     */
    updateNeeds() {
        const recentSignals = this.signalHistory.getRecent(this.config.trendWindowSize);
        const newNeeds = [];
    ...
    computeNeedScore(rule, signals) {
        ...
        for (const signalData of signals) {
            ...
            if (weight !== undefined) {
                ...
                const decayedIntensity = signal.intensity * Math.pow(this.config.decayFactor, age / 1000);
     ...
     */
    calculateSignalTrend(signalType, windowSize = this.config.trendWindowSize) {
    ...
    getStatistics() {
        ...
        return {
            ...
            averageSignalAge: allSignals.length > 0 ? totalAge / allSignals.length : 0,
        };
    }
    /**
     ...
     */
    calculateNeeds(signals, context) {
        ...
        for (const rule of this.aggregationRules) {
            ...
            if (signalCount > 0) {
                ...
                if (context && rule.contextGates) {
                    for (const [contextKey, multiplier] of Object.entries(rule.contextGates)) {
    ...
    aggregateSignals(signals) {
        ...
        for (const [type, typeSignals] of signalMap.entries()) {
            if (typeSignals.length === 1) {
                ...
                const avgIntensity = typeSignals.reduce((sum, s) => sum + s.intensity, 0) /
                    typeSignals.length;
                const avgConfidence = typeSignals.reduce((sum, s) => sum + s.confidence, 0) /
                    typeSignals.length;
                const avgTrend = typeSignals.reduce((sum, s) => sum + s.trend, 0) / typeSignals.length;
     ...
     */
    calculatePriority(need, context) {
     ...
     */
    generateGoalCandidates(needs, worldState) {
        const goals = [];
     ...
     */
    checkFeasibility(goal, worldState) {
        const missing = [];
