 ...
 */
class ConstitutionalFilter extends events_1.EventEmitter {
    constructor() {
        super();
        this.rules = new Map();
        ...
        const defaultRules = [
            ...
            },
            {
                ...
                evaluate: async (capability, request, context) => {
                    ...
                    const nearPlayers = context.nearbyEntities.filter((e) => e.type === 'player' && e.distance < 50);
            ...
            },
            {
                ...
                evaluate: async (capability, request, context) => {
                    const nearPlayers = context.nearbyEntities.filter((e) => e.type === 'player' && e.distance < 20);
                    if (nearPlayers.length > 0 &&
            ...
            },
            {
                ...
                evaluate: async (capability, request, context) => {
                    const isEmergency = context.agentHealth < 0.2 || context.dangerLevel > 0.8;
     ...
     */
    async evaluateExecution(capability, request, context) {
        const applicableRules = this.getApplicableRules(capability, request, context);
        const violatedRules = [];
        const evaluations = [];
     ...
     */
    getApplicableRules(capability, request, context) {
        const rules = [];
        for (const [id, rule] of this.rules) {
            if (!rule.enabled)
                continue;
            // Check if rule applies to this capability
            const appliesToCapability = rule.appliesTo.includes('*') ||
            ...
            if (rule.safetyTagTriggers.length > 0) {
                const hasTriggeredTag = rule.safetyTagTriggers.some((tag) => capability.safetyTags.includes(tag));
                if (!hasTriggeredTag)
                    continue;
            }
            // Check context conditions
            if (rule.contextConditions.length > 0) {
                const contextMatches = rule.contextConditions.some((condition) => this.evaluateContextCondition(condition, context));
                if (!contextMatches)
                    continue;
            }
            rules.push(rule);
    ...
    evaluateContextCondition(condition, context) {
        ...
        switch (condition.type) {
            ...
            case 'time':
                ...
                break;
            case 'social':
                contextValue = context.nearbyEntities.filter((e) => e.type === 'player').length;
                break;
            default:
                return false;
        }
     ...
     */
    recordViolation(ruleId, severity) {
     ...
     */
    async wouldApprove(capability, request, context) {
...
//# sourceMappingURL=constitutional-filter.js.map