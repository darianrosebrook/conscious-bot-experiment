...
const capability_specs_1 = require("./capability-specs");
/**
 ...
 */
class CapabilityRegistry extends events_1.EventEmitter {
    constructor() {
        super();
    ...
    initializeDefaultCapabilities() {
        for (const capability of capability_specs_1.ALL_CAPABILITIES) {
            this.registerCapability(capability);
        }
        // Register executors
        for (const [capabilityId, executor] of Object.entries(capability_specs_1.CAPABILITY_EXECUTORS)) {
            this.executors.set(capabilityId, executor);
        }
        // Register validators
        for (const [capabilityId, validator] of Object.entries(capability_specs_1.CAPABILITY_VALIDATORS)) {
    ...
    registerCapability(spec) {
        try {
            ...
            const validatedSpec = (0, types_1.validateCapabilitySpec)(spec);
            // Check for conflicts
     ...
     */
    registerExecutor(capabilityId, executor) {
        this.executors.set(capabilityId, executor);
    }
    /**
     ...
     */
    registerValidator(capabilityId, validator) {
        this.validators.set(capabilityId, validator);
    }
    /**
    ...
    discoverCapabilities(query) {
        ...
        for (const [id, capability] of this.capabilities) {
            ...
            if (query.riskLevel !== undefined &&
                ...
                score += 0.2;
                reasons.push(`risk level acceptable: ${capability.riskLevel} <= ${query.riskLevel}`);
            }
            // Safety tags filter
            if (query.safetyTags && query.safetyTags.length > 0) {
                const matchingTags = query.safetyTags.filter((tag) => capability.safetyTags.includes(tag));
            ...
            if (query.maxDuration !== undefined &&
                ...
                score += 0.15;
                reasons.push(`duration within limit: ${capability.durationMs}ms <= ${query.maxDuration}ms`);
            }
            // Cost filter
            if (query.maxCost !== undefined && capability.costHint <= query.maxCost) {
                score += 0.15;
                reasons.push(`cost within limit: ${capability.costHint} <= ${query.maxCost}`);
            }
            // Text search
            if (query.searchText) {
     ...
     */
    isCapabilityAvailable(capabilityId) {
        const capability = this.capabilities.get(capabilityId);
        if (!capability || !capability.enabled)
            return false;
        // Check concurrent execution limit
        const activeCount = Array.from(this.activeExecutions.values()).filter((req) => req.capabilityId === capabilityId).length;
     ...
     */
    async validateExecution(request, context) {
        try {
        ...
        if (validator) {
            try {
                preconditionsPassed = await validator.validatePreconditions(request, context);
                ...
                preconditionsPassed = false;
                reasons.push(`Precondition validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        // Basic constitutional check (simplified)
        if (capability.riskLevel >= types_1.RiskLevel.HIGH && !request.metadata?.approved) {
            constitutionalApproval = false;
        ...
        const approved = preconditionsPassed && constitutionalApproval && rateLimitApproval;
        if (approved) {
    ...
    async executeCapability(request, context) {
        ...
        try {
            ...
            if (!executor) {
                throw new Error(`No executor found for capability: ${request.capabilityId}`);
            }
            // Execute capability
