 ...
 */
export function validateLeafSpec(spec) {
 ...
 */
export function validateLeafImpl(impl) {
    if (!impl.spec) {
 ...
 */
export function mapExceptionToErrorCode(error) {
 ...
 */
export function createExecError(error) {
 ...
 */
export function createLeafContext(bot, abortSignal) {
 ...
 */
export function getPermissionIntersection(leaves) {
    if (leaves.length === 0)
        return [];
 ...
 */
export async function verifyPostconditions(postSchema, before, after, ajv) {
    if (!postSchema)
        return { ok: true };
