...
class CognitiveStreamValidationTest {
  ...
  private async testCapabilityRegistration(): Promise<void> {
    ...
    try {
      ...
      this.testDetails.set('capability_registration', 
        hasCapabilities 
      ...
      const torchCapability = capabilities.find((cap: any) => cap.name === 'opt.torch_corridor');
      this.testResults.set('torch_capability', !!torchCapability);
      this.testDetails.set('torch_capability',
        torchCapability 
  ...
  private async testGoalIdentification(): Promise<void> {
    ...
    try {
      ...
      this.testDetails.set('goal_identification',
        hasGoals 
  ...
  private async testPlanningExecution(): Promise<void> {
    ...
    try {
      ...
      await this.integration.executePlanningCycle('torch the mining corridor safely');
      ...
      this.testDetails.set('planning_execution',
        hasPlanning 
      ...
      const mcpPlanning = events.find(e => e.metadata?.approach === 'mcp-capabilities');
      this.testResults.set('mcp_planning', !!mcpPlanning);
      this.testDetails.set('mcp_planning',
        mcpPlanning 
  ...
  private async testEventStreaming(): Promise<void> {
    ...
    try {
      ...
      this.testDetails.set('event_streaming',
        hasEvents 
      ...
      const hasCapabilities = events.some(e => e.type === 'capability');
  ...
  private async testMCPIntegration(): Promise<void> {
    ...
    try {
      ...
      const capabilities = await this.integration.getMCPCapabilities();
      const activeCapabilities = capabilities.filter((cap: any) => cap.status === 'active');
      const shadowCapabilities = capabilities.filter((cap: any) => cap.status === 'shadow');

      this.testResults.set('mcp_registry', capabilities.length > 0);
      this.testDetails.set('mcp_registry',
      ...
      this.testDetails.set('leaf_factory',
        `✅ Leaf Factory: ${leaves.length} leaves registered`
      );

      // Check for required leaves
      const requiredLeaves = ['move_to', 'sense_hostiles', 'retreat_and_block', 'place_torch_if_needed', 'step_forward_safely'];
      const missingLeaves = requiredLeaves.filter(leaf => !leaves.find((l: any) => l.name === leaf));
      
      this.testResults.set('required_leaves', missingLeaves.length === 0);
      this.testDetails.set('required_leaves',
  ...
  private async testSafetyFeatures(): Promise<void> {
    ...
    try {
      ...
      const safetyGoals = events.filter(e => 
      ...
      this.testDetails.set('safety_goals',
      ...
      const emergencyGoals = events.filter(e => 
      ...
      this.testDetails.set('emergency_response',
  ...
  private printResults(): void {
    ...
    if (failedTests === 0) {
      console.log('🎉 All tests passed! Cognitive Stream Integration is working correctly.');
    } else {
      console.log(`⚠️ ${failedTests} test(s) failed. Review the implementation.`);
    }
    ...
    const complianceChecks = [
      { name: 'MCP Capabilities Integration', passed: this.testResults.get('mcp_integration') },
      { name: 'Goal Identification System', passed: this.testResults.get('goal_identification') },
      { name: 'Planning Execution Flow', passed: this.testResults.get('planning_execution') },
      { name: 'Event Streaming', passed: this.testResults.get('event_streaming') },
      { name: 'Safety Features', passed: this.testResults.get('safety_features') },
    ];
