...
exports.HybridHRMArbiter = void 0;
 ...
 */
class HybridHRMArbiter extends arbiter_1.Arbiter {
    constructor(hybridHRMConfig, performanceBudgets) {
        super();
        this.signalHistory = [];
        this.needHistory = [];
        this.goalTemplates = [];
        ...
        this.hybridHRM = new hybrid_hrm_integration_1.HybridHRMRouter(hybridHRMConfig);
    ...
    setCachedResult(task, system, result) {
        this.cache.set(task, {
            task,
            system,
            result,
    ...
    shouldSkipLLM(signals) {
        ...
        return signals.every((signal) => {
            ...
            const isUrgent = signal.name.includes('threat') || signal.name.includes('health');
     ...
     */
    async processGoalsParallel(candidates, skipLLM = false) {
        const promises = candidates.map(async (candidate) => {
            ...
            try {
                ...
                const result = await this.hybridHRM.reason(taskDescription, this.currentContext, {
                ...
                console.warn(`HRM planning failed for ${candidate.template.name}:`, error);
                return candidate;
            }
        });
    ...
    async initialize() {
        ...
        try {
            ...
            console.warn('⚠️ Hybrid HRM initialization failed, falling back to basic mode');
            return false;
        }
        catch (error) {
     ...
     */
    async processHRMSignal(signal, context) {
    ...
    async processHRMSignalOptimized(signals, context) {
        ...
        try {
            ...
            if (batches.length > 1) {
                console.log(`📦 Processing ${signals.length} signals in ${batches.length} optimized batches`);
            }
            const allResults = [];
            // Process each batch
            for (const batch of batches) {
                ...
                if (batch.skipLLM) {
                    ...
                    console.log(`🚀 Smart routing: Skipping LLM for batch ${batch.batchIndex + 1}`);
                }
                // OPTIMIZATION 4: Parallel processing
                const plannedGoals = await this.processGoalsParallel(rankedGoals.slice(0, 3), batch.skipLLM);
                if (performance.now() - planningTime >
                    this.performanceBudgets.routine.hrmPlanning) {
                    console.warn('⚠️ HRM planning exceeded budget');
                }
                const batchTime = performance.now() - batchStartTime;
                console.log(`📦 Batch ${batch.batchIndex + 1} completed in ${batchTime.toFixed(1)}ms`);
    ...
    logOptimizationStats() {
        if (this.optimizationStats.batchesCreated > 1 ||
    ...
    async computeNeeds(signals, context) {
        const needs = [];
        // Safety need
        const safetySignals = signals.filter((s) => s.provenance === 'env' && s.name.includes('threat'));
     ...
     */
    async computeSafetyNeed(signals, context) {
        const threatProximity = signals.find((s) => s.name === 'threatProximity')?.value || 0;
        ...
        const baseScore = threatProximity * 0.4 +
            (1 - lightLevel / 15) * 0.3 +
            (1 - health / 20) * 0.3;
        // Context gates
        ...
        return {
            name: 'Safety',
            score: Math.min(1.0, baseScore * timeGate * locationGate * socialGate * environmentalGate),
            dScore: 0,
     ...
     */
    async computeProgressNeed(signals) {
        const toolDeficit = signals.find((s) => s.name === 'toolDeficit')?.value || 0;
        const questBacklog = signals.find((s) => s.name === 'questBacklog')?.value || 0;
     ...
     * Compute social need
     */
    async computeSocialNeed(signals) {
        const playerNearby = signals.find((s) => s.name === 'playerNearby')?.value || 0;
        const isolationTime = signals.find((s) => s.name === 'isolationTime')?.value || 0;
     ...
     */
    computeCuriosityNeed(signals) {
        const novelty = signals.find((s) => s.name === 'novelty')?.value || 0;
        const unexploredFrontier = signals.find((s) => s.name === 'unexploredFrontier')?.value || 0;
     ...
     */
    async enumerateGoals(needs, context) {
        const candidates = [];
    ...
    cleanupExpiredSignals() {
        const now = Date.now();
        this.signalHistory = this.signalHistory.filter((signal) => !signal.ttlMs || now - signal.timestamp < signal.ttlMs);
    }
    /**
     * Plan with HRM for top candidates (now uses parallel processing)
     */
    async planWithHRM(candidates) {
    ...
    initializeGoalTemplates() {
        ...
        this.goalTemplates.push({
            ...
            preconditions: async (context) => {
                ...
                const food = inventory.find((i) => i.name.includes('food'));
...
exports.HybridHRMArbiter = HybridHRMArbiter;
/**
 * HRM Cognitive Module for integration with main Arbiter
 */
class HRMCognitiveModule {
    constructor(hybridHRM) {
        this.hybridHRM = hybridHRM;
        this.type = arbiter_1.ModuleType.HRM;
        this.name = 'hrm-cognitive';
...
//# sourceMappingURL=hybrid-hrm-arbiter.js.map