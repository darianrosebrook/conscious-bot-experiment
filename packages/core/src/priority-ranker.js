...
exports.PriorityRanker = exports.RankingMethod = exports.OpportunityType = exports.CommitmentType = exports.RiskLevel = exports.ResourceType = exports.TaskType = void 0;
const events_1 = require("events");
const uuid_1 = require("uuid");
var TaskType;
(function (TaskType) {
    TaskType["SURVIVAL"] = "survival";
...
(function (CommitmentType) {
    ...
    CommitmentType["GOAL"] = "goal";
    CommitmentType["OBLIGATION"] = "obligation";
    CommitmentType["EXPECTATION"] = "expectation";
...
(function (OpportunityType) {
    ...
    OpportunityType["LEARNING"] = "learning";
    OpportunityType["EXPLORATION"] = "exploration";
    OpportunityType["ACHIEVEMENT"] = "achievement";
    OpportunityType["CREATIVE"] = "creative";
})(OpportunityType || (exports.OpportunityType = OpportunityType = {}));
var RankingMethod;
(function (RankingMethod) {
    RankingMethod["WEIGHTED_SUM"] = "weighted_sum";
    RankingMethod["MULTI_CRITERIA"] = "multi_criteria";
    RankingMethod["ANALYTIC_HIERARCHY"] = "analytic_hierarchy";
    RankingMethod["FUZZY_LOGIC"] = "fuzzy_logic";
    RankingMethod["MACHINE_LEARNING"] = "machine_learning";
    RankingMethod["HYBRID"] = "hybrid";
})(RankingMethod || (exports.RankingMethod = RankingMethod = {}));
...
// ============================================================================
class PriorityRanker extends events_1.EventEmitter {
    constructor(config = {}) {
        super();
        this.tasks = new Map();
    ...
    async calculateTaskPriority(task, context) {
        ...
        const opportunityCostBoost = await this.calculateOpportunityCostBoost(task, context);
        priorityFactors.push({
        ...
        const resourceAvailability = this.calculateResourceAvailability(task, context);
        priorityFactors.push({
            name: 'resource_availability',
            weight: this.config.resourceAvailabilityWeight,
            value: resourceAvailability,
            contribution: resourceAvailability * this.config.resourceAvailabilityWeight,
        ...
        const calculatedPriority = Math.min(1.0, Math.max(0, priorityFactors.reduce((sum, factor) => sum + factor.contribution, 0)));
        // Generate ranking reason
        const rankingReason = this.generateRankingReason(priorityFactors, calculatedPriority);
        return {
            ...task,
            calculatedPriority,
            commitmentBoost,
     ...
     */
    async calculateCommitmentBoost(task) {
        if (!this.config.enableCommitmentTracking) {
            return 0;
        }
        const relevantCommitments = Array.from(this.commitments.values()).filter((commitment) => commitment.targetTask === task.id ||
     ...
     */
    calculateNoveltyBoost(task, context) {
        ...
        if (context.socialContext.includes('new') ||
     ...
     */
    async calculateOpportunityCostBoost(task, context) {
        if (!this.config.enableOpportunityTracking) {
            return 0;
        }
        const relevantOpportunities = Array.from(this.opportunities.values()).filter((opportunity) => opportunity.description
            .toLowerCase()
            .includes(task.name.toLowerCase()) ||
            opportunity.requirements.some((req) => task.resources.some((res) => res.name.includes(req))));
        ...
        for (const opportunity of relevantOpportunities) {
            ...
            const urgencyFactor = Math.max(0.1, 1 - timeRemaining / (opportunity.timeWindow * 60000));
     ...
     */
    calculateResourceAvailability(task, context) {
     ...
     */
    calculateSocialImpact(task, context) {
        // Base social value from metadata
        let impact = task.metadata.socialValue;
        // Adjust based on social context
        if (context.socialContext.includes('group') ||
        ...
        const socialGoals = context.currentGoals.filter((goal) => goal.toLowerCase().includes('social') ||
     ...
     */
    calculateLearningValue(task, context) {
     ...
     */
    calculateRiskLevel(task, context) {
        let riskScore = 0;
     ...
     */
    calculateRiskPenalty(riskLevel) {
        switch (riskLevel) {
     ...
     */
    calculateFeasibility(task, context) {
        let feasibility = 0.5; // Base feasibility
        // Resource feasibility
        const resourceFeasibility = this.calculateResourceAvailability(task, context);
     ...
     */
    generateRankingReason(factors, priority) {
     ...
     * Calculate ranking metadata
     */
    calculateRankingMetadata(tasks) {
     ...
     */
    createPriorityHistogram(priorities) {
        const buckets = 10;
    ...
    identifyTopFactors(tasks) {
        ...
        return Array.from(factorMap.values()).sort((a, b) => b.contribution - a.contribution);
    }
    /**
     * Calculate ranking confidence
     */
    calculateRankingConfidence(tasks) {
        if (tasks.length < 2)
            return 1.0;
        // Calculate confidence based on factor consistency
        const factorConsistencies = tasks[0].priorityFactors.map((factor) => {
            const values = tasks.map((t) => t.priorityFactors.find((f) => f.name === factor.name)?.value || 0);
            const variance = this.calculateVariance(values);
            return Math.max(0, 1 - variance);
        });
        return (factorConsistencies.reduce((sum, c) => sum + c, 0) /
     ...
     */
    calculateRankingQuality(tasks) {
        if (tasks.length < 2)
            return 1.0;
        // Quality based on priority spread and factor diversity
        const priorities = tasks.map((t) => t.calculatedPriority);
        const spread = Math.max(...priorities) - Math.min(...priorities);
        const factorDiversity = new Set(tasks.flatMap((t) => t.priorityFactors.map((f) => f.name))).size;
    ...
    getStats() {
        return {
            ...
            averageRankingConfidence: this.rankingHistory.length > 0
...
class CommitmentTracker {
    ...
    getCommitmentsForTask(taskId) {
        return Array.from(this.commitments.values()).filter((c) => c.targetTask === taskId);
    }
    updateCommitmentStrength(commitmentId, strength) {
...
class OpportunityTracker {
    ...
    getExpiredOpportunities() {
        const now = Date.now();
        return Array.from(this.opportunities.values()).filter((o) => o.expiresAt < now);
    }
