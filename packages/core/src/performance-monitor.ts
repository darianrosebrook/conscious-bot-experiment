...
class LatencyTracker {
  ...
  getStatistics(): {
    ...
    const recent = this.measurements
      .getRecent(this.measurements.size())
      .map((item) => item.value)
      .sort((a, b) => a - b);
...
export class PerformanceMonitor extends EventEmitter<SystemEvents> {
  ...
  constructor(
    private config: PerformanceMonitorConfig = DEFAULT_PERFORMANCE_CONFIG
  ) {
    super();
   ...
   */
  startTracking(
    operation: CognitiveTask,
    context: 'emergency' | 'routine' | 'deliberative' = 'routine'
  ): TrackingSession {
  ...
  private handleBudgetViolation(session: TrackingSession): void {
    ...
    const violation: SafetyViolation = {
      ...
      severity:
        this.consecutiveViolations >= this.config.violationThreshold
          ? 'high'
          : 'medium',
    ...
    if (
      this.config.degradationEnabled &&
      this.consecutiveViolations >= this.config.violationThreshold
    ) {
  ...
  attemptRecovery(): void {
    ...
    if (shouldRecover) {
      this.currentDegradation = Math.max(
        this.currentDegradation - 1,
  ...
  private generateCurrentMetrics(): PerformanceMetrics {
    ...
    const allStats = Array.from(this.latencyTrackers.values()).map((tracker) =>
    ...
    return {
      ...
      quality: {
        ...
        errorRate:
          this.consecutiveViolations / Math.max(1, this.operationCounter),
      },
    };
  }
  ...
  private performPeriodicCheck(): void {
    ...
    for (const [sessionId, session] of this.activeSessions) {
      if (session.getElapsed() > session.budget.total * 2) {
        console.warn(
          `Session ${sessionId} appears stalled (${session.getElapsed()}ms)`
        );
  ...
  private estimateCpuUtilization(): number {
    ...
    const latencyLoad = Math.min(
      metrics.latency.p95 / this.config.budgets.routine,
      1
    );
