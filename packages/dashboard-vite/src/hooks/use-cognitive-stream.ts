      ...
      es.onmessage = (event) => {
        ...
        try {
          ...
          if (
            ...
            // Use GOAL-stripped canonical content as key (ignoring type) so
            // DRIVE-TICK "My inventory is bare" and INTERNAL "My inventory
            // is bare [GOAL: collect oak_log 8]" collapse to one entry.
            if (payload.type === 'cognitive_stream_init') {
              const seen = new Map<string, CognitiveThought>();
              for (const t of thoughts) {
                const cleaned = (t.displayContent || t.content || '')
                  .replace(GOAL_TAG_RE, '')
                  .replace(/\s+/g, ' ')
                  .trim()
                  .toLowerCase();
                seen.set(cleaned, t); // later occurrence overwrites earlier
              }
