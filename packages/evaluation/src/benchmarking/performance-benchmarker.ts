...
export const BenchmarkSuiteResultSchema = z.object({
  ...
  agentRankings: z.array(z.object({
    agentId: z.string(),
    rank: z.number(),
    score: z.number(),
  ...
  // Statistical analysis
  statisticalSignificance: z.record(z.object({
    pValue: z.number(),
    effectSize: z.number(),
...
export class PerformanceBenchmarker extends EventEmitter {
    ...
    try {
      ...
      for (const agentId of config.agents) {
        for (const scenarioId of config.scenarios) {
          ...
          this.emit('benchmark_combination_completed', { agentId, scenarioId, result });
        }
      }

      // Perform statistical analysis
      const statisticalSignificance = this.statisticalAnalyzer.analyzeSignificance(results);
  ...
  private async executeBenchmarkCombination(
      ...
      try {
        ...
        if (!isWarmup) {
          rawResults.push({
            ...
            success: false,
            metrics: [],
            planningPerformance: { latency: 0, qualityScore: 0, refinementCount: 0, routingDecisions: [] },
            executionPerformance: { latency: 0, accuracyScore: 0, adaptationCount: 0, errorRate: 1 },
            cognitivePerformance: { memoryUtilization: 0, reasoningDepth: 0, coherenceScore: 0, creativityScore: 0 },
            strengths: [],
    ...
    const completionRate = rawResults.filter(r => r.overallScore > 0).length / rawResults.length;
    const errorRate = 1 - completionRate;

    // Calculate quality metrics
    const planningQuality = this.calculateAverageMetric(rawResults, 'planningPerformance.qualityScore');
    const executionQuality = this.calculateAverageMetric(rawResults, 'executionPerformance.accuracyScore');
    const adaptabilityScore = this.calculateAverageMetric(rawResults, 'executionPerformance.adaptationCount') / 10; // Normalize
  ...
  private calculateStatistics(values: number[]): {
    ...
    if (values.length === 0) {
      return { mean: 0, median: 0, stdDev: 0, min: 0, max: 0, percentile95: 0, percentile99: 0 };
    }
   ...
   */
  private calculateAverageMetric(results: EvaluationResults[], metricPath: string): number {
  ...
  private generateScenarioAnalysis(results: BenchmarkResult[]): any[] {
    const scenarioStats = new Map<string, { scores: number[]; difficulties: number[] }>();
    ...
    return Array.from(scenarioStats.entries()).map(([scenarioId, stats]) => {
      const avgPerformance = stats.scores.reduce((sum, s) => sum + s, 0) / stats.scores.length;
      const variance = stats.scores.reduce((sum, s) => sum + Math.pow(s - avgPerformance, 2), 0) / stats.scores.length;
   ...
   */
  private analyzePerformanceTrends(results: BenchmarkResult[]): Record<string, any> {
  ...
  private generateBenchmarkReport(suiteResult: BenchmarkSuiteResult): string {
    ...
    report.push(`**Generated:** ${new Date(suiteResult.timestamp).toISOString()}`);
    report.push(`**Execution Time:** ${(suiteResult.executionTime / 1000).toFixed(1)}s`);
    report.push('');

    // Executive Summary
    report.push('## Executive Summary');
    report.push('');
    report.push(`- **Overall Score:** ${(suiteResult.overallScore * 100).toFixed(1)}%`);
    report.push(`- **Success Rate:** ${(suiteResult.overallSuccessRate * 100).toFixed(1)}%`);
    report.push(`- **Total Iterations:** ${suiteResult.totalIterations}`);
    report.push(`- **Agents Tested:** ${suiteResult.config.agents.length}`);
    report.push(`- **Scenarios Tested:** ${suiteResult.config.scenarios.length}`);
    report.push('');
    ...
    suiteResult.agentRankings.forEach(ranking => {
      ...
      report.push(`- **Strengths:** ${ranking.strengths.join(', ') || 'None identified'}`);
      report.push(`- **Weaknesses:** ${ranking.weaknesses.join(', ') || 'None identified'}`);
      report.push('');
    });
    ...
    if (suiteResult.regressionSummary.totalRegressions > 0) {
      ...
      report.push('');
      report.push(`- **Total Regressions:** ${suiteResult.regressionSummary.totalRegressions}`);
      report.push(`- **Severe Regressions:** ${suiteResult.regressionSummary.severeRegressions}`);
      report.push(`- **Affected Agents:** ${suiteResult.regressionSummary.affectedAgents.join(', ')}`);
      report.push(`- **Affected Scenarios:** ${suiteResult.regressionSummary.affectedScenarios.join(', ')}`);
      report.push('');
    ...
    suiteResult.scenarioAnalysis
      ...
      .forEach(analysis => {
        ...
        report.push(`- **Difficulty:** ${(analysis.difficulty * 10).toFixed(1)}/10`);
        report.push(`- **Average Performance:** ${(analysis.averagePerformance * 100).toFixed(1)}%`);
        report.push(`- **Discriminative Power:** ${(analysis.discriminativePower * 100).toFixed(1)}%`);
        report.push('');
      });

    report.push('---');
    report.push('*Report generated by the Conscious Bot Performance Benchmarking System*');
   ...
   */
  private storeBenchmarkResult(benchmarkId: string, result: BenchmarkSuiteResult): void {
  ...
  private convertToCSV(data: BenchmarkSuiteResult[]): string {
    ...
    const headers = ['Benchmark ID', 'Timestamp', 'Overall Score', 'Success Rate', 'Execution Time'];
...
class RegressionDetector {
  async detectRegressions(
    ...
    currentResults.forEach(result => {
      ...
      if (baseline) {
        const degradation = (baseline.successRate - result.successRate) / baseline.successRate;
    ...
    return {
      ...
      recommendations: this.generateRegressionRecommendations(regressions, severeRegressions)
    };
  }
  ...
  analyzeRegression(
    ...
    const currentSuccessRate = currentResults.filter(r => r.success).length / currentResults.length;
    const degradation = (baseline.successRate - currentSuccessRate) / baseline.successRate;
  ...
  private generateRegressionRecommendations(regressions: string[], severeRegressions: string[]): string[] {
    const recommendations: string[] = [];
    
    if (severeRegressions.length > 0) {
      recommendations.push('Investigate severe performance regressions immediately');
