...
describe('Ray-Casting Property-Based Tests', () => {
    ...
    test('ray casting is deterministic for same inputs', () => {
      fc.assert(fc.property(
        ...
        ({ origin, direction, maxDistance }) => {
          ...
          const result1 = raycastEngine.castRay(origin, direction, maxDistance);
          const result2 = raycastEngine.castRay(origin, direction, maxDistance);

          // Property: Same inputs produce identical outputs
          expect(result1.hit).toBe(result2.hit);
          if (result1.hit && result2.hit) {
            expect(result1.hitPosition).toEqual(result2.hitPosition);
    ...
    test('ray distance never exceeds maximum distance', () => {
      fc.assert(fc.property(
        ...
        ({ origin, direction, maxDistance }) => {
          ...
          const result = raycastEngine.castRay(origin, direction, maxDistance);
  ...
  describe('Visible-Only Sensing Properties', () => {
      ...
      fc.assert(fc.property(
        fc.array(
          fc.record({
            ...
            blockType: fc.constantFrom('minecraft:stone', 'minecraft:coal_ore', 'minecraft:air')
          }),
    ...
    test('confidence decay follows exponential properties', () => {
      fc.assert(fc.property(
        fc.record({
          ...
          decayRate: fc.float({ min: 0.001, max: 0.1 })
        }),
    ...
    test('field of view constraints are respected', () => {
      fc.assert(fc.property(
        fc.record({
          agentYaw: fc.float({ min: 0, max: 2 * Math.PI }),
          agentPitch: fc.float({ min: -Math.PI / 2, max: Math.PI / 2 }),
          targetPosition: fc.record({
            x: fc.float({ min: -50, max: 50 }),
            y: fc.float({ min: 40, max: 100 }),
        ...
        ({ agentYaw, agentPitch, targetPosition, fovDegrees }) => {
          ...
          const dotProduct = facing.x * toTarget.x + facing.y * toTarget.y + facing.z * toTarget.z;
