  ...
  switch (task.type) {
    ...
    case 'gathering':
      return {
        type: 'gather',
        parameters: {
          resource: task.title.toLowerCase().includes('wood') ? 'wood' : 'resource',
          amount: 3,
          target: task.title.toLowerCase().includes('wood') ? 'tree' : 'resource',
        },
        timeout: 15000,
      };
    case 'gather':
      return {
        type: 'gather',
        parameters: {
          resource: task.parameters?.resource || (task.title.toLowerCase().includes('wood') ? 'wood' : 'resource'),
    ...
    case 'mining':
      return {
        type: 'mine_block',
        parameters: {
          blockType: task.title.toLowerCase().includes('iron') ? 'iron_ore' : 'stone',
    ...
    default:
      logOptimizer.warn(`⚠️ No action mapping for task type: ${task.type}`, `no-action-mapping-${task.type}`);
      return null;
  }
}

export function mapBTActionToMinecraft(tool: string, args: Record<string, any>) {
  const debugInfo = { originalAction: tool, args: args };
  switch (tool) {
    case 'scan_for_trees':
      return { type: 'wait', parameters: { duration: 2000 } };
    case 'pathfind':
      return { type: 'move_forward', parameters: { distance: args.distance || 1 } };
    case 'scan_tree_structure':
      return { type: 'wait', parameters: { duration: 1000 } };
    case 'dig_blocks':
      return { type: 'dig_block', parameters: { pos: args.position || 'current', tool: args.tool || 'axe' } };
    case 'collect_items':
      return { type: 'pickup_item', parameters: { radius: args.radius || 3 } };
    case 'clear_3x3_area':
      return {
        type: 'mine_block',
        parameters: { position: args.position || 'current', tool: args.tool || 'pickaxe', area: { x: 3, y: 2, z: 3 } },
        debug: debugInfo,
      };
    case 'place_blocks': {
      ...
      if (pattern === '3x3_floor') {
        return { type: 'place_block', parameters: { block_type: blockType, count: 9, placement: 'pattern_3x3_floor' } };
      } else if (pattern === '3x3_walls_2_high') {
        return { type: 'place_block', parameters: { block_type: blockType, count: 12, placement: 'pattern_3x3_walls' } };
      } else if (pattern === '3x3_roof') {
        return { type: 'place_block', parameters: { block_type: blockType, count: 9, placement: 'pattern_3x3_roof' } };
      } else {
        return { type: 'place_block', parameters: { block_type: blockType, count: 1, placement: 'around_player' } };
      }
    }
    case 'place_door':
      return { type: 'place_block', parameters: { block_type: 'oak_door', count: 1, placement: 'specific_position', position: args.position || 'front_center' } };
    case 'place_torch':
      return {
        type: 'place_block',
        parameters: {
          block_type: 'torch',
          count: 1,
          placement: args.position === 'center_wall' ? 'specific_position' : 'around_player',
