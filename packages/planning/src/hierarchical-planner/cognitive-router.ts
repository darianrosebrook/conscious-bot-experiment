...
export class CognitiveTaskRouter {
  ...
  private classifyTask(context: TaskContext): TaskType {
    ...
    if (this.isCreativeTask(inputLower) || requiresCreativity) return 'creative_task';
    ...
    if (requiresWorldKnowledge && !requiresStructured) return 'natural_language';
   ...
   */
  private selectRouter(taskType: TaskType, context: TaskContext): 'llm' | 'hrm_structured' | 'collaborative' {
    ...
    if (['navigation', 'logic_puzzle', 'resource_optimization', 'spatial_reasoning', 'pattern_recognition'].includes(taskType)) {
    ...
    if (['natural_language', 'social_interaction', 'creative_task', 'open_ended_query'].includes(taskType)) {
      return 'llm';
    }
   ...
   */
  private calculateConfidence(taskType: TaskType, context: TaskContext): number {
   ...
   */
  private generateReasoning(taskType: TaskType, context: TaskContext, router: string): string {
    const routerMap = {
    ...
    const urgencyNote = context.urgency === 'emergency' 
      ? ' (emergency: prioritizing speed)' 
      : '';
   ...
   */
  private getTaskCharacteristics(taskType: TaskType, context: TaskContext): string {
    if (['navigation', 'logic_puzzle', 'spatial_reasoning'].includes(taskType)) {
   ...
   */
  recordTaskResult(decision: RoutingDecision, success: boolean, actualLatency: number): void {
    ...
    current.avgLatency = ((current.avgLatency * (current.total - 1)) + actualLatency) / current.total;
  ...
  getRoutingStats(): {
    ...
    for (const [key, metrics] of this.performanceMetrics.entries()) {
      accuracyByTaskType[key] = metrics.total > 0 ? metrics.success / metrics.total : 0;
    }
    ...
    for (const decision of this.routingHistory) {
      if (!routerLatencies[decision.router]) routerLatencies[decision.router] = [];
    ...
    for (const [router, latencies] of Object.entries(routerLatencies)) {
      avgLatencyByRouter[router] = latencies.reduce((a, b) => a + b, 0) / latencies.length;
    }
 ...
 */
export function routeTask(input: string, options: Partial<TaskContext> = {}): RoutingDecision {
