 ...
 */
export const defaultUtilityFn: UtilityFunction = {
  id: 'default-utility',
  ...
  calculate: (ctx: UtilityContext): number => {
    ...
    return clamp(
      intensity * 0.5 + urgency * 0.3 + novelty * 0.1 + opportunity * 0.1
    );
  },
};
...
export class GoalManager {
  ...
  createFromNeeds(
    needs: Need[],
    utilityFn: UtilityFunction = defaultUtilityFn
  ): Goal | undefined {
    if (!needs?.length) return undefined; // Early return guard
  ...
  selectNext(): Goal | undefined {
    if (!this.goals.length) return undefined;
    const candidates = this.goals.filter(
      (g) =>
        g.status === GoalStatus.PENDING || g.status === GoalStatus.SUSPENDED
    );
