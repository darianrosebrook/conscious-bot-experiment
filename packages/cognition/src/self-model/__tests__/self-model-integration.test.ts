...
describe('Self-Model Integration', () => {
  ...
  describe('Identity Tracking', () => {
    ...
    it('should reinforce personality traits based on evidence', () => {
      ...
      const updatedCuriousTrait = updatedTraits.find(
        (t) => t.name === 'Curious'
      );

      expect(updatedCuriousTrait!.strength).toBeGreaterThan(initialStrength);
      expect(updatedCuriousTrait!.evidence.length).toBe(
        initialEvidenceCount + 1
      );
      expect(updatedCuriousTrait!.evidence).toContain(
        'Explored new cave system for 30 minutes'
      );
    });
    ...
    it('should update value consistency based on behavior alignment', () => {
      ...
      const updatedSafetyValue = updatedValues.find(
        (v) => v.name === 'Safety First'
      );

      expect(updatedSafetyValue!.consistency).toBeGreaterThan(
        initialConsistency
      );
      expect(updatedSafetyValue!.manifestations).toContain(
        'Avoided dangerous cliff area during exploration'
      );
    });
    ...
    it('should develop capabilities through experience', () => {
      ...
      const initialMiningCapCount = initialCapabilities.filter(
        (c) => c.name === 'Mining'
      ).length;
      ...
      const miningCapability = updatedCapabilities.find(
        (c) => c.name === 'Mining'
      );
    ...
    it('should process identity impacts from experiences', () => {
      ...
      expect(() =>
        identityTracker.processIdentityImpact(impacts)
      ).not.toThrow();
    ...
    it('should generate meaningful identity summary', () => {
      ...
      identityTracker.developCapability('Building', 0.6, 'Built a house', [
        'Used wood and stone',
      ]);
      identityTracker.reinforcePersonalityTrait(
        'Helpful',
  ...
  describe('Narrative Management', () => {
    ...
    it('should integrate experiences into narrative context', () => {
      ...
      const description =
        'Discovered a beautiful valley with flowing streams and wildlife';
      const context = {
        timestamp: Date.now(),
    ...
    it('should track narrative themes and patterns', () => {
      ...
      narrativeManager.startNewStory(
        'Learning Journey',
  ...
  describe('Integrated Self-Model Functionality', () => {
    it('should maintain coherent self-understanding across identity and narrative', () => {
      ...
      const experienceDescription =
        'Successfully learned advanced building techniques through trial and error';
      ...
      const buildingCap = identity.capabilities.find(
        (c) => c.name === 'Advanced Building'
      );
      expect(buildingCap).toBeDefined();
      expect(buildingCap!.proficiency).toBe(0.4);

      const persistentTrait = identity.personalityTraits.find(
        (t) => t.name === 'Persistent'
      );
    ...
    it('should handle value conflicts and resolutions in narrative context', () => {
      ...
      const conflictExperience =
        'Chose safer but slower route despite time pressure';
      ...
      const safetyValue = freshTracker
        .getCoreValues()
        .find((v) => v.name === 'Safety First');
      const initialConsistency = safetyValue!.consistency;
      ...
      expect(integration.lessonsExtracted).toContain(
        'Caution prevents harmful consequences'
      );

      const narrativeSummary = narrativeManager.generateNarrativeSummary();
      expect(narrativeSummary).toContain('safer');

      const updatedSafetyValue = freshTracker
        .getCoreValues()
        .find((v) => v.name === 'Safety First');
      expect(updatedSafetyValue!.consistency).toBeGreaterThan(
        initialConsistency
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle invalid inputs gracefully', () => {
      ...
      expect(() =>
        identityTracker.reinforcePersonalityTrait('', 'test', 0.1)
      ).not.toThrow();
      expect(() =>
        identityTracker.updateValueConsistency('invalid-id', true, 'test')
      ).not.toThrow();
      expect(() => identityTracker.processIdentityImpact([])).not.toThrow();

      // Narrative manager error cases
      expect(() =>
        narrativeManager.integrateExperience('', '', { timestamp: Date.now() })
