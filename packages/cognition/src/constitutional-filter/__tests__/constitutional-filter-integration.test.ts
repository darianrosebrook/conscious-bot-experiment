...
describe('Constitutional Filter Integration', () => {
  ...
  describe('Rules Database', () => {
    ...
    it('should retrieve rules by category', () => {
      ...
      expect(safetyRules.every((r) => r.category === RuleCategory.SAFETY)).toBe(
        true
      );

      const ethicsRules = rulesDB.getRulesByCategory(RuleCategory.ETHICS);
      expect(ethicsRules.length).toBeGreaterThan(0);
      expect(ethicsRules.every((r) => r.category === RuleCategory.ETHICS)).toBe(
        true
      );
    });
  ...
  describe('Action Filtering', () => {
    ...
    it('should modify actions when appropriate', async () => {
      ...
      const result = await constitutionalFilter.filterAction(
        actionNeedingModification
      );
  ...
  describe('Message Filtering', () => {
    it('should allow respectful messages', async () => {
      const respectfulMessage: MessageProposal = {
        id: 'test-message-1',
        content:
          'I have analyzed your code and found several ways to improve performance.',
        recipient: 'user',
        intent: 'provide_assistance',
      };

      const result =
        await constitutionalFilter.filterMessage(respectfulMessage);
    ...
    it('should modify disrespectful messages', async () => {
      ...
      const result =
        await constitutionalFilter.filterMessage(disrespectfulMessage);
