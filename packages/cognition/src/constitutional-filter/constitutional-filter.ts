...
export class ConstitutionalFilter {
  ...
  async filterAction(
    ...
    const { allowed, modifiedAction } = this.enforceActionDecision(action, result);
  ...
  async filterMessage(
    ...
    // Apply enforcement based on result
    const { allowed, modifiedMessage } = this.enforceMessageDecision(message, result);
  ...
  async filterIntrusion(
    ...
    // Evaluate intrusion
    const result = await this.rulesEngine.evaluateIntrusion(intrusion, contextData);
  ...
  private enforceActionDecision(
    ...
    switch (this.config.enforcementLevel) {
      case EnforcementLevel.STRICT:
        ...
        } else if (result.decision === RuleAction.MODIFY && this.config.autoCorrect) {
          // Apply modifications if auto-correct is enabled
          if (result.suggestedModifications && result.suggestedModifications.length > 0) {
      ...
      case EnforcementLevel.STANDARD:
        ...
        if (result.decision === RuleAction.ALLOW) {
          ...
          // Apply modifications if auto-correct is enabled
          if (this.config.autoCorrect && result.suggestedModifications && result.suggestedModifications.length > 0) {
            // Add modifications to justification
            modifiedAction.justification = modifiedAction.justification ?
              `${modifiedAction.justification}\n\nModified due to constitutional rules: ${result.suggestedModifications.join(', ')}` :
              `Modified due to constitutional rules: ${result.suggestedModifications.join(', ')}`;
      ...
      case EnforcementLevel.ADVISORY:
        ...
        if (result.decision === RuleAction.DENY || result.decision === RuleAction.ESCALATE) {
          // Add warning to justification
          modifiedAction.justification = modifiedAction.justification ?
            `${modifiedAction.justification}\n\nWarning: This action violates constitutional rules but is permitted in advisory mode.` :
            `Warning: This action violates constitutional rules but is permitted in advisory mode.`;
        } else if (result.decision === RuleAction.MODIFY && result.suggestedModifications) {
    ...
    switch (this.config.enforcementLevel) {
      ...
      case EnforcementLevel.STANDARD:
        ...
        if (result.decision === RuleAction.ALLOW) {
          ...
          if (result.warningFlags && result.warningFlags.length > 0) {
            const constraints = modifiedGoal.constraints || [];
            constraints.push(...result.warningFlags.map(flag => `Warning: ${flag}`));
            modifiedGoal.constraints = constraints;
          }
          return { allowed: true, modifiedGoal };
        } else {
      ...
      case EnforcementLevel.ADVISORY:
        ...
        if (result.decision === RuleAction.DENY || result.decision === RuleAction.ESCALATE) {
          ...
          const constraints = modifiedGoal.constraints || [];
          constraints.push('Warning: This goal violates constitutional rules but is permitted in advisory mode.');
          modifiedGoal.constraints = constraints;
        } else if (result.decision === RuleAction.MODIFY && result.suggestedModifications) {
          // Add suggestions to constraints
          const constraints = modifiedGoal.constraints || [];
          constraints.push(...result.suggestedModifications.map(mod => `Suggestion: ${mod}`));
          modifiedGoal.constraints = constraints;
        } else if (result.decision === RuleAction.FLAG && result.warningFlags) {
          // Add warning flags to constraints
          const constraints = modifiedGoal.constraints || [];
  ...
  private enforceMessageDecision(
    ...
    switch (this.config.enforcementLevel) {
      ...
      case EnforcementLevel.ADVISORY:
        ...
        if (result.decision === RuleAction.DENY || result.decision === RuleAction.ESCALATE) {
          // Add warning prefix to message
          const warning = '[Warning: This message violates constitutional rules but is permitted in advisory mode.] ';
          modifiedMessage.content = warning + modifiedMessage.content;
        } else if (result.decision === RuleAction.MODIFY && result.suggestedModifications) {
   ...
   */
  private applyMessageModification(content: string, suggestion: string): string {
  ...
  private enforceIntrusionDecision(
    ...
    switch (this.config.enforcementLevel) {
      ...
      case EnforcementLevel.STANDARD:
        ...
        return result.decision === RuleAction.ALLOW || result.decision === RuleAction.FLAG;
  ...
  async generateComplianceReport(): Promise<ComplianceReport> {
    ...
    const evaluations = this.rulesEngine.getEvaluationHistory()
      .filter(evalResult => evalResult.timestamp >= start && evalResult.timestamp <= now);
    
    // Calculate statistics
    const totalEvaluations = evaluations.length;
    const violationCount = evaluations.filter(evalResult => 
      evalResult.decision === RuleAction.DENY || 
      evalResult.decision === RuleAction.ESCALATE
    ).length;
    
    const flaggedCount = evaluations.filter(evalResult => 
      evalResult.decision === RuleAction.FLAG
    ).length;
    
    const escalationCount = evaluations.filter(evalResult => 
      evalResult.decision === RuleAction.ESCALATE
    ).length;
    
    // Calculate compliance by category
    const byCategory: Record<RuleCategory, number> = Object.values(RuleCategory)
      .reduce((acc, category) => {
        acc[category] = 0;
        return acc;
    ...
    for (const evalResult of evaluations) {
      for (const rule of evalResult.appliedRules) {
        if (rule.action === RuleAction.DENY || rule.action === RuleAction.ESCALATE) {
    ...
    const significantViolations = evaluations
      .filter(evalResult => 
        (evalResult.decision === RuleAction.DENY || evalResult.decision === RuleAction.ESCALATE) &&
    ...
    const overallCompliance = totalEvaluations > 0 ?
      1 - ((violationCount + escalationCount + flaggedCount * 0.5) / totalEvaluations) :
      1;
    ...
    if (sortedCategories.length > 0) {
      for (const [category, count] of sortedCategories) {
        recommendations.push(`Review compliance with ${category} rules (${count} violations).`);
      }
    }
    
    if (escalationCount > 0) {
      recommendations.push(`Address ${escalationCount} escalated issues requiring human oversight.`);
    }
    
    if (overallCompliance < 0.9) {
      recommendations.push('Consider additional constitutional rule training to improve compliance.');
    }
