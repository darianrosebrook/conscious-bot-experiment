...
export class RelationshipExtractor {
      ...
      try {
        ...
        const locationEntity = await this.graphRAG.addEntity(
          experience.location.name ||
            `Location-${experience.location.x}-${experience.location.y}-${experience.location.z}`,
          EntityType.PLACE,
          {
  ...
  private extractEntityMatches(text: string): Array<{
    ...
    const objectPattern =
      /(?:a|an|the)\s+([a-zA-Z0-9\s]+)(?:\s+(?:is|was|that is|that was))/gi;
    let objectMatch;
    ...
    const placePattern =
      /(?:at|in|to|from)\s+(?:a|an|the)?\s+([a-zA-Z0-9\s]+)(?:\s+(?:where|which|that))?/gi;
    let placeMatch;
    ...
    const creaturePattern =
      /(?:a|an|the)\s+([a-zA-Z0-9\s]+)(?:\s+(?:creature|mob|animal|monster))/gi;
    let creatureMatch;
   ...
   */
  private extractRelationshipMatches(
    text: string,
    entities: Entity[]
  ): Array<{
    sourceId: string;
