...
describe('Semantic Memory Integration', () => {
  ...
  describe('Knowledge Graph Core', () => {
    ...
    it('should add and retrieve relationships', () => {
      ...
      const retrievedRelationship = knowledgeGraph.getRelationship(
        relationship.id
      );
  ...
  describe('GraphRAG', () => {
    ...
    it('should format query results in different formats', async () => {
      // Add entities
      await graphRAG.addEntity('Oak Wood', EntityType.ITEM, {
        hardness: 2,
      });

      await graphRAG.addEntity('Oak Planks', EntityType.ITEM, {
        hardness: 2,
      });
  ...
  describe('Relationship Extractor', () => {
    it('should extract entities and relationships from text', async () => {
      const text =
        'A diamond is a valuable gem. A diamond pickaxe is used for mining obsidian.';

      const extraction = await relationshipExtractor.extractFromText(text);

      expect(extraction.entities.length).toBeGreaterThan(0);
      expect(extraction.entities.some((e) => e.name.includes('diamond'))).toBe(
        true
      );
    ...
    it('should extract entities and relationships from experience', async () => {
      ...
      const extraction =
        await relationshipExtractor.extractFromExperience(experience);

      expect(extraction.entities.length).toBeGreaterThan(0);
      expect(
        extraction.entities.some(
          (e) => e.name === 'Deep Cave' || e.type === EntityType.PLACE
        )
      ).toBe(true);
  ...
  describe('Integration', () => {
    it('should build and query a knowledge graph about Minecraft items', async () => {
      ...
      await graphRAG.addEntity('Diamond', EntityType.ITEM, { durability: 10 });

      await graphRAG.addEntity('Diamond Pickaxe', EntityType.ITEM, {
        durability: 1561,
      });

      await graphRAG.addEntity('Obsidian', EntityType.BLOCK, { hardness: 50 });
      ...
      const queryResult = await queryEngine.query(
        'How is Diamond related to Obsidian?'
      );
