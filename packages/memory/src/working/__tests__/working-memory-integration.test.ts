...
describe('Working Memory Integration', () => {
  ...
  describe('Context Manager', () => {
    ...
    it('should update multiple context types', () => {
      ...
      contextManager.updateTemporalContext({ currentTime: Date.now(), dayPhase: 'day' });
    ...
    it('should get most relevant contexts', () => {
      contextManager.updateSpatialContext({ biome: 'mountains' }, { relevance: 0.9 });
      contextManager.updateTemporalContext({ currentTime: Date.now() }, { relevance: 0.5 });
  ...
  describe('Goal Tracker', () => {
    it('should add goals to working memory', () => {
      const result = goalTracker.addGoal(
        'Find food',
        { priority: 0.8 }
      );
    ...
    it('should manage goal status transitions', () => {
      ...
      expect(goalTracker.getGoalsByStatus(GoalStatus.COMPLETED)).toHaveLength(1);
    });
    ...
    it('should handle subgoals', () => {
      ...
      const subgoalResult = goalTracker.addSubgoal(
        parentId,
        'Gather wood'
      );
  ...
  describe('Memory Integration', () => {
    ...
    it('should add verbal information to phonological loop', () => {
      ...
      expect(state.buffers.phonological[0].content).toBe('Turn left at the big oak tree');
    });

    it('should rehearse verbal information', () => {
      const addResult = memoryIntegration.addVerbalInfo('Remember to collect water');
      const itemId = addResult.affectedItems[0];
    ...
    it('should create inferences from working memory items', () => {
      ...
      const state = centralExecutive.getState();
      const inferenceItem = state.workingItems.find(i => i.type === WorkingItemType.INFERENCE);
    ...
    it('should provide cognitive workspace snapshot', () => {
      ...
      contextManager.updateSpatialContext({ biome: 'forest', landmarks: ['river'] });
      centralExecutive.addItem(WorkingItemType.OBSERVATION, 'Rain clouds forming');
  ...
  describe('Integration and Edge Cases', () => {
    ...
    it('should maintain attention focus when handling distractions', () => {
      ...
      const handledDistraction = updatedState.attentionFocus.distractions
        .find(d => d.source === distractionId);
